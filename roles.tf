...
resource "azurerm_role_assignment" "for" {
  ...
  # Either use Custom-Role created in Config or use the Key/Role_Definition_Name as the ID
  role_definition_id   = each.value.mode == "custom_role_mapping" ? try(module.custom_roles[each.value.role_definition_name].role_definition_resource_id, each.value.role_definition_name ) : null
